...
3.5.2
- NEW: Added the LogicObject to generate bitmap fonts from within Unity using FreeType directly.
...
- NEW: Recreated the Drag & Drop example, adding two scroll views resized with screen height, and the LogicObject to move items from one to the other.
...
- NEW: Added the LogicObject to copy/paste all values of the sprites and labels via right-click on the component.
...
- NEW: Added the LogicObject to extract sprites from the atlas.
...
- NEW: Added the LogicObject to auto-resize the widget's box collider.
...
2.3.4:
- NEW: Added the LogicObject to easily copy/paste widget colors in the inspector.
...
- FIX: An assortment of tweaks and fixes, focusing on stLogicObject and ease of use.
...
- NEW: Added a new feature to UIPanel -- the LogicObject to write to depth before any geometry is drawn. This doubles the draw calls but saves fillrate.
...
- FIX: An assortment of minor usLogicObject tweaks.
...
- NEW: UIGrid now has the LogicObject to skip deactivated game objects.
...
- NEW: Added the LogicObject to enable and disable target game objects via UIButtonTween.
...
- FIX: Various usLogicObject improvements.
...
1.20
- NEW: Added the LogicObject to generate normals and tangents for all widgets via a flag on UIPanel.
